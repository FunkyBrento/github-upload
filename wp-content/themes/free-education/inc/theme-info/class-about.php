...
if ( ! class_exists( 'Free_Education_About' ) ) {
	...
	class Free_Education_About {
		...
		/**
		 * Singleton instance of Free_Education_About.
		 *
		 * @var Free_Education_About $instance Free_Education_About instance.
		 */
		private static $instance;

		/**
		 * Main Free_Education_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Free_Education_About ) ) {
				self::$instance = new Free_Education_About;
		...
		public function setup_config() {
			...
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'free-education' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'free-education' ), $this->theme_name ) . '</a></p>' );
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'free-education' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'free-education' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'free-education' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'free-education' );
